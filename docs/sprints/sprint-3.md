...
| 3.11 | Verification sweep | `meta/verify-sweep` | `pnpm audit-services` green; manual spot-check of 30 random pages logged | 3.12 |
...
2. **Manual spot-check log** — 30 services chosen by `shuf -n 30 src/content/services/ | head -30`, each walked via the Visual Verification Protocol. Result logged at `docs/sprints/sprint-3-verification-log.md` (one line per service: slug + pass/fail + note).
3. **Lighthouse smoke** — `/`, `/catalog`, one `/service/*`, one `/category/*` all return mobile score ≥ 90 (Sprint 5 raises the bar to 98).
4. **Build artifact diff** — `du -sh dist/` before and after the sprint, logged for future tuning.
...
| Free-tier limits changed since April 2026 (≥30 days stale) | Med | Med | Spot-check 30 random services in story 3.11; bump `date_verified` to 2026-05 only after re-check |
...
## §10 — Sprint-Level Definition of Done

- [ ] Every service checkbox in §6 is ticked
- [ ] Every category gate box in §6 is ticked
- [ ] `pnpm lint && pnpm validate && pnpm check-logos && pnpm audit-services && pnpm build` all green on `rebuild/astro`
